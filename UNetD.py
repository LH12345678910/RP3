...
def conv2d_block(input_tensor, n_filters, drop, filter_size):
  x = input_tensor
  for i in range(2):
...
def encoder_block(inputs, drop, n_filters, filter_size, pool_size):
  c = conv2d_block(inputs, n_filters, drop, filter_size)
  p = tf.keras.layers.MaxPooling2D(pool_size = (pool_size, pool_size))(c)

  return c, p


def encoder(inputs, drop, n_filters, filter_size, pool_size):
  c1, p1 = encoder_block(inputs, drop, n_filters, filter_size, pool_size)
  c2, p2 = encoder_block(p1, drop, n_filters*2, filter_size, pool_size)
  c3, p3 = encoder_block(p2, drop, n_filters*4, filter_size, pool_size)
  c4, p4 = encoder_block(p3, drop, n_filters*8, filter_size, pool_size)

  return p4, (c1, c2, c3, c4)

def bottleneck(inputs, n_filters, drop, filter_size):
  bottle_neck = conv2d_block(inputs, n_filters*16, drop, filter_size)

  return bottle_neck

def decoder_block(inputs, conv_output, drop, n_filters, filter_size, stride):
  ...
  c = conv2d_block(c, n_filters, drop, filter_size)

  return c


def decoder(inputs, convs, drop, n_filters, filter_size, stride, output_channels):
  c1, c2, c3, c4 = convs

  c6 = decoder_block(inputs, c4, drop, n_filters*8, filter_size, stride)
  c7 = decoder_block(c6, c3, drop, n_filters*4, filter_size, stride)
  c8 = decoder_block(c7, c2, drop, n_filters*2, filter_size, stride)
  c9 = decoder_block(c8, c1, drop, n_filters, filter_size, stride)
...
def unet(drop, n_filters, filter_size, pool_size, stride, output_channels):
  inputs = tf.keras.layers.Input(shape=(128, 128,1,))

  encoder_output, convs = encoder(inputs, drop, n_filters, filter_size, pool_size)

  bottle_neck = bottleneck(encoder_output, n_filters, drop, filter_size)

  outputs = decoder(bottle_neck, convs, drop, n_filters, filter_size, stride, output_channels)
  model = tf.keras.Model(inputs=inputs, outputs=outputs)

  return model
tf.keras.backend.clear_session()

model = unet(drop=0.0853, n_filters=16, filter_size=3, pool_size=2, stride=2, output_channels=3)
...
model.compile(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=0.0012,momentum=0.9865,nesterov=True), loss=combined_loss,
              metrics=[DL1,
                       DL2,
                       DL3,
